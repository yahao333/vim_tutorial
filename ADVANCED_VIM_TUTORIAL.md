...
Vim的命令行窗口是一个强大的工具，它允许您查看、编辑和重用命令历史和搜索历史，从而提高您的工作效率。

### 使用 `q:` 和 `q/` 进行历史编辑

-   **`q:` (命令历史)**: 在普通模式下，按下 `q`，然后按下 `:`，Vim会打开一个新窗口，其中包含您最近执行的所有命令。您可以在这个窗口中像编辑普通文本一样编辑这些命令，然后按下 `<CR>` 执行修改后的命令。

-   **`q/` (搜索历史)**: 类似地，按下 `q`，然后按下 `/`，Vim会打开一个新窗口，其中包含您最近执行的所有搜索模式。您可以在这个窗口中编辑搜索模式，然后按下 `<CR>` 执行修改后的搜索。

**示例：**

1.  在Vim中执行一些命令，例如 `:set number`，`:w`，`:q`。
2.  按下 `q:`，您会看到这些命令出现在一个新窗口中。
3.  您可以移动光标到某个命令上，修改它，然后按下 `<CR>` 执行。

通过 `q:` 和 `q/`，您可以轻松地重用和修改复杂的命令或搜索模式，避免重复输入，提高效率。

## 第五步：全局命令 (`:g`)

Vim的 `:global` 命令（简写为 `:g`）是一个非常强大的工具，它允许您在文件中查找匹配特定模式的行，并对这些行执行命令。这使得批量操作变得异常高效。
...
- `[range]`: 可选，指定命令作用的行范围。如果省略，则作用于整个文件。
- `{pattern}`: 要匹配的正则表达式模式。Vim会在文件中查找所有匹配这个模式的行。
- `{command}`: 对匹配到的每一行要执行的Vim命令。这个命令可以是任何Ex命令（以 `:` 开头的命令）。
...
这会查找所有包含 "TODO" 的行，并删除它们。

### 结合其他命令

`:g` 命令的强大之处在于它可以与几乎所有Ex命令结合使用。

**示例：将所有包含 "function" 的行复制到文件末尾**
...
3.  您可以移动光标到某个命令上，修改它，然后按下 `<CR>` 执行。
 ...
 表示文件末尾。这会将匹配到的行复制到文件末尾。

**示例：在所有包含 "error" 的行前面添加 "[FIXME] "**

```vim
:g/error/normal! I[FIXME] 
```

- `normal!`: 强制执行普通模式命令。`I` 是在行首插入文本。
...
: 匹配空行。
- `s/$/\r\r/`: 在行尾添加两个回车符，从而在每个空行后添加两个新行。

### 反向全局命令 (`:v`)

`:global` 命令还有一个反向版本，即 `:vglobal`（简写为 `:v`）。它会对**不匹配**指定模式的行执行命令。

**示例：删除所有不包含 "import" 的行**

```vim
:v/import/d
```

`:global` 命令是Vim中进行批量文本处理的利器，熟练掌握它将极大地提升您的编辑效率。
