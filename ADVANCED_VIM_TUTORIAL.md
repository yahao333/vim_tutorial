...
- [写在前面：别把Vim用成Sublime](#写在前面别把vim用成sublime)
- [第一章：文本操作的“道”](#第一章文本操作的道)
  - [第一节：文本对象 - Vim的“语法”](#第一节文本对象---vim的语法)
  - [第二节：寄存器 - Vim的“多功能剪贴板”](#第二节寄存器---vim的多功能剪贴板)
  - [第三节：正则表达式 - 文本处理的“核武器”](#第三节正则表达式---文本处理的核武器)
- [第二章：工作流的“术”](#第二章工作流的术)
  - [第一节：宏 - 你的“私人自动化助理”](#第一节宏---你的私人自动化助理)
  - [第二节：全局命令 (`:g`) - 终极批处理命令](#第二节全局命令-g---终极批处理命令)
  - [第三节：命令行窗口 (`q:`) - 告别重复输入](#第三节命令行窗口-q---告别重复输入)
  - [第四节：窗口与缓冲区 - 管理你的“工作区”](#第四节窗口与缓冲区---管理你的工作区)
- [第三章：Vim的“魂”](#第三章vim的魂)
  - [第一节：与Shell共舞 - `!`的威力](#第一节与shell共舞----的威力)
  - [第二节：自动命令 (`autocmd`) - 让Vim“活”起来](#第二节自动命令-autocmd---让vim活起来)
  - [第三节：Vimscript 进阶 - 打造你的“专属武器”](#第三节vimscript-进阶---打造你的专属武器)
  - [第四节：插件与插件管理器 - 站在巨人的肩膀上](#第四节插件与插件管理器---站在巨人的肩膀上)
- [总结：成为工具的主人](#总结成为工具的主人)
...
如果你还在用简单的字符串搜索和替换，那你的Vim只发挥了不到10%的功力。正则表达式，才是Vim处理文本的真正“核武器”。它让你能够以模式匹配的方式，精准地定位和操作文本。

Vim的正则表达式语法有些“独特”，但一旦掌握，你会发现它异常强大。

- `\v`: "very magic" 模式。推荐使用！它让正则表达式语法更接近你熟悉的PCRE（Perl Compatible Regular Expressions），减少反斜杠的噩梦。
- `\m`: "magic" 模式。Vim默认模式，很多特殊字符需要加反斜杠转义。
- `\V`: "very nomagic" 模式。几乎所有字符都解释为字面意思，特殊字符需要加反斜杠。

**实战思维：**
- **场景1：批量修改函数签名**
  假设你有一个旧的API，函数签名是 `old_func(arg1, arg2)`，现在要改成 `new_func(arg2, arg1)`。
  `:s/\vold_func\(([^,]+), ([^)]+)\)/new_func(\2, \1)/g`
  - `\v`: 开启 very magic 模式。
  - `([^,]+)`: 捕获第一个参数（非逗号字符的一个或多个）。
  - `([^)]+)`: 捕获第二个参数（非右括号字符的一个或多个）。
  - `\2, \1`: 在替换时交换捕获组的顺序。

- **场景2：删除所有空行和注释行**
  `:g/^\s*\($\|\/\/\|#\)/d`
  - `^\s*`: 行首的零个或多个空格。
  - `\($\|\/\/\|#\)`: 匹配行尾（空行）、`//`（C/C++/Java注释）或 `#`（Python/Shell注释）。
  - `d`: 删除匹配的行。

**正则表达式是Vim的灵魂之一。掌握它，你才能真正理解Vim的强大，才能在文本的海洋中自由穿梭，无所不能。**
...
你是不是经常在Vim的命令行里敲错命令，或者想重复执行之前某个复杂的命令？然后一遍遍地按上下箭头，或者重新输入？

`q:` 命令，就是你的救星！它会打开一个独立的窗口，里面列出了你所有的历史命令。你可以像编辑普通文本一样，在里面修改、复制、粘贴、执行任何历史命令。这比在小小的命令行里折腾高效百倍。

**实战思维：**
1.  按下 `q:`，一个新窗口会弹出，里面是你的命令历史。
2.  找到你之前敲错的那个 `:s/old/new/g` 命令。
3.  像编辑普通文本一样，用 `cw` 修改 `old` 为 `correct_old`。
4.  光标移动到修改后的那一行，按下回车 (`<CR>`)，Vim就会执行这条修正后的命令。

**这不仅仅是方便，这是一种对“重复”的零容忍。Vim的哲学就是：任何重复性的操作，都应该被自动化或简化。**

### 第四节：窗口与缓冲区 - 管理你的“工作区”

别再像用记事本一样，一次只开一个文件。也别像某些IDE一样，开几十个标签页，然后用鼠标点来去。Vim有更高效的方式管理你的工作区：**窗口 (Window)** 和 **缓冲区 (Buffer)**。
...
### 第三节：Vimscript 进阶 - 打造你的“专属武器”

如果你满足于使用别人写好的插件，那你就永远只是个“使用者”。Vimscript，才是让你从“使用者”变成“创造者”的钥匙。它是Vim的内置编程语言，让你能够编写自己的函数、命令，甚至更复杂的逻辑，把Vim打造成完全符合你工作流的“专属武器”。

- **变量作用域**: `g:` (全局), `b:` (缓冲区), `w:` (窗口), `t:` (标签页), `s:` (脚本), `l:` (局部), `a:` (函数参数), `v:` (Vim预定义)。理解这些，才能写出健壮的代码。
- **数据类型**: 列表 `[]` 和字典 `{}`。Vimscript也支持这些现代编程语言的特性。
- **控制流**: `if-else`, `for`, `while`。基本的编程逻辑。
- **错误处理**: `try-catch`。让你的脚本更健壮。

**实战思维：**
假设你经常需要把当前文件路径复制到系统剪贴板，但Vim没有直接的命令。你可以自己写一个：
```vim
" 定义一个函数，复制当前文件路径到系统剪贴板
def! CopyFilePath()
  let @+ = expand("%:p")
  echo "文件路径已复制到剪贴板: " . @+
enddef
...
```
现在，你只需要在Vim中输入 `:CopyPath`，当前文件的绝对路径就会被复制到系统剪贴板。**这，就是Vimscript的魅力——让Vim为你所用，而不是你被Vim所困。**

### 第四节：插件与插件管理器 - 站在巨人的肩膀上

虽然我们鼓励自己动手，但没必要重复造轮子。Vim拥有一个庞大而活跃的插件生态系统。插件管理器，就是让你高效管理这些“巨人肩膀”的工具。

- **为什么需要插件管理器？**
  手动安装插件是噩梦。插件管理器帮你自动化下载、安装、更新、卸载插件，解决依赖冲突，让你的 `.vimrc` 保持整洁。
- **推荐：`vim-plug`**
  轻量、快速、异步。这是目前最主流的插件管理器之一。
...
2.  **在 `.vimrc` 中配置插件**:
    ...
    call plug#end()
    ```
3.  **安装插件**: 保存 `.vimrc`，重新打开Vim，执行 `:PlugInstall`。

**插件不是万能药，但它们能极大地扩展Vim的能力边界。学会选择和配置高质量的插件，是成为Vim高手的必经之路。但切记，不要被插件“绑架”，核心能力永远是Vim本身。**

---

## 总结：成为工具的主人

我们讲了Vim的“道”（文本对象、寄存器、正则表达式）、“术”（宏、全局命令、命令行窗口、窗口与缓冲区）和“魂”（与Shell共舞、自动命令、Vimscript、插件）。你应该已经能体会到其设计思想的强大。Vim不是要你死记硬背，而是希望你理解它的“语言”，并用它来创造性地解决问题。
